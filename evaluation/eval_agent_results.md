...
| Metric | Layer | Result |
|---|---|---|
| **Execution accuracy** (result set == gold) | 1 | **100%** (23/23 runs) |
| Mean result-set overlap (partial credit) | 1 | 100% |
| Substring pass rate (the v1/v2 metric) | 1 | 98% |
| Term-scoping compliance | 1 | 6% (2/36 answered runs) |
...
| **Task success** (right answer, safely) | 3 | **100%** |
| Refusal rate on must-refuse cases | 3 | 100% |
| Latency p50 / p95 | 3 | 9.9s / 11.2s |
| Mean tokens per query | 3 | 2882 |
...
| **Total (regression set)** | 39 | 100% | 100% |
...
| `adv_011` term_scoping | 'this term' is explicit, so omitting the term filter mixes archived 2024 sections into the answer | 1/1 | answered, 42 rows |
